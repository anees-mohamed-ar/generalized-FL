...
class NeuralNetwork(BaseEstimator, ClassifierMixin):
    ...
    def apply_gradients(self, gradients):
        """Apply gradient updates and print before/after changes to ensure updates are working."""
        print("\n🔎 Debug: Weights Before Applying Gradients (First 5 values)")
        print(f"W1: {self.W1.flatten()[:5]}")
        print(f"W2: {self.W2.flatten()[:5]}")
        print(f"W3: {self.W3.flatten()[:5]}")

        # ✅ Apply gradients directly to weights
        ...
        print("\n🔎 Debug: Weights After Applying Gradients (First 5 values)")
        print(f"W1: {self.W1.flatten()[:5]}")
        print(f"W2: {self.W2.flatten()[:5]}")
        print(f"W3: {self.W3.flatten()[:5]}")
...
class CKKSKeyManager:
    ...
    def load_keys(self):
        ...
        try:
            ...
            try:
                with open(f"{self.save_dir}ckks_context.bin", "rb") as f:
                    context_data = f.read()
                self.context = ts.context_from(context_data)  # Load full context, including secret key
...
def encrypt_gradients(gradients, context):
    """Encrypt gradients using the encryption context"""
    encrypted_grads = {}
    for key, grad in gradients.items():
        original_shape = grad.shape
        flattened = grad.flatten()
...
def decrypt_gradients(encrypted_gradients, decryption_context):
    """Decrypt gradients using the decryption context"""
    decrypted_grads = {}
    for key, grad_dict in encrypted_gradients.items():
        # Get the encrypted data and shape
        encrypted_data = grad_dict['data']
        original_shape = grad_dict['shape']
        
        # Load the encrypted data into the decryption context if needed
...
class FederatedLearning:
    def __init__(self, input_dim, num_rounds=10, learning_rate=0.01, model_path="global_model.json",local_epochs =5):
        ...
        self.context = self.key_manager.generate_keys()  # Generate and store keys
        self.key_manager.save_keys()  # Save keys for future use
        self.momentum = 0.9
        self.previous_gradients = None
        self.local_epochs =local_epochs

    def save_global_gradients(self, gradients):
        """Save the global gradients instead of weights to a JSON file."""
        try:
            gradient_data = {
                'dW1': gradients['dW1'].tolist(),
                'db1': gradients['db1'].tolist(),
                'dW2': gradients['dW2'].tolist(),
                'db2': gradients['db2'].tolist(),
                'dW3': gradients['dW3'].tolist(),
                'db3': gradients['db3'].tolist(),
                'learning_rate': self.learning_rate,
                'momentum': self.momentum
            }
            with open(self.model_path, 'w') as f:
                json.dump(gradient_data, f)
            print(f"✅ Saved global gradients to global_gradients.json")
        except Exception as e:
            print(f"❌ Error saving global gradients: {e}")

    def load_global_gradients(self):
        """Load the stored gradients and apply them to update the global model."""
        try:
            with open(self.model_path, 'r') as f:
                gradient_data = json.load(f)

            gradients = {
                'dW1': np.array(gradient_data['dW1']),
                'db1': np.array(gradient_data['db1']),
                'dW2': np.array(gradient_data['dW2']),
                'db2': np.array(gradient_data['db2']),
                'dW3': np.array(gradient_data['dW3']),
                'db3': np.array(gradient_data['db3']),
            }
            self.learning_rate = gradient_data['learning_rate']
            self.momentum = gradient_data['momentum']


            print(f"✅ Loaded global gradients from global_gradients.json")

            # Apply gradients to update the model
            self.global_model.apply_gradients(gradients)

        except FileNotFoundError:
            print("🚨 No existing gradients found. Starting fresh.")
        except Exception as e:
            print(f"❌ Error loading global gradients: {e}")
        ...
        while True:
            ...
            try:
                ...
                if accuracy > best_accuracy:
                    ...
                    self.save_global_gradients(avg_gradients) 
                else:
    ...
    def fit(self, X_train_nodes, y_train_nodes, X_test, y_test, centralized_accuracy, loop_until_surpass,local_epochs):
        ...
        while True:
            ...
            try:
                ...
                avg_gradients = decrypt_gradients(avg_encrypted_gradients,decryption_context)
                avg_gradients = clip_gradients(avg_gradients, max_norm=1.0)
