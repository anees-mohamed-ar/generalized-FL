...
def compute_privacy_metrics(original_gradients, encrypted_gradients, decrypted_gradients, encryption_time, decryption_time):
    """
    ...
    for key in original_gradients:
        original_norm = np.linalg.norm(original_gradients[key]) + 1e-8  # Avoid division by zero
        decrypted_norm = np.linalg.norm(decrypted_gradients[key])
        encrypted_norm = np.linalg.norm(encrypted_gradients[key].data.decrypt())  # Decrypt for comparison
    ...
    if plm_score < 0.02:
        console.print("[bold green]✓ Excellent Privacy Protection![/]")
    elif plm_score < 0.05:
        console.print("[bold yellow]⚠ Moderate Privacy Protection. Needs Improvement.[/]")
    else:
        console.print("[bold red]❌ High Privacy Leakage! Consider stronger encryption.[/]")
        ...
        try:
            ...
            for epoch in range(self.local_epochs):
                ...
                for i in range(0, n_samples, self.batch_size):
                    ...
                    if self.original_gradients is None:
