...
class FederatedNode(threading.Thread):
    ...
    def compute_privacy_metrics(self):
        """Compute privacy metrics after local training."""
        if self.gradients is None:
            self.console.print(f"[red]Node {self.node_id}: No gradients to compute privacy metrics.[/]")
            return None
        
        # Convert dictionary of gradients into a single vector
        original_grads = np.concatenate([v.flatten() for v in self.gradients.values()])
        encrypted_grads = np.concatenate([v.decrypt().flatten() for v in self.encrypted_gradients.values()])
        
        # Compute privacy metrics
        plm = self.compute_privacy_leakage(original_grads, encrypted_grads)
        gss = self.compute_gradient_similarity(original_grads, encrypted_grads)
        msd = self.compute_msd(original_grads, encrypted_grads)
        
        self.console.print(f"\n[bold cyan]Node {self.node_id} Privacy Metrics:[/]")
        self.console.print(f"🔒 [green]Privacy Leakage Metric (PLM):[/] {plm:.4f}")
        self.console.print(f"🔒 [green]Gradient Similarity Score (GSS):[/] {gss:.4f}")
        self.console.print(f"🔒 [green]Mean Squared Difference (MSD):[/] {msd:.6f}")
        
        return {"PLM": plm, "GSS": gss, "MSD": msd}
    
    def compute_privacy_leakage(self, original_gradients, encrypted_gradients):
        """Calculate privacy leakage using entropy difference."""
        from scipy.stats import entropy
        original_entropy = entropy(np.abs(original_gradients).flatten())
        encrypted_entropy = entropy(np.abs(encrypted_gradients).flatten())

        return max(0, original_entropy - encrypted_entropy)

    def compute_gradient_similarity(self, original_gradients, encrypted_gradients):
        """Calculate cosine similarity between original and encrypted gradients."""
        from sklearn.metrics.pairwise import cosine_similarity
        original = original_gradients.reshape(1, -1)
        encrypted = encrypted_gradients.reshape(1, -1)
        return cosine_similarity(original, encrypted)[0, 0]

    def compute_msd(self, original_gradients, encrypted_gradients):
        """Calculate mean squared difference between original and encrypted gradients."""
        import numpy as np
        return np.mean((original_gradients - encrypted_gradients) ** 2)
    ...
    def run(self):
        try:
            ...
            # Compute privacy metrics after training
            self.encrypted_gradients = encrypt_gradients(self.gradients, self.encryption_context)
            self.privacy_metrics = self.compute_privacy_metrics()
...
class EnhancedFederatedLearning(FederatedLearning):
    ...
    def fit(self, X_train_nodes, y_train_nodes, X_test, y_test, centralized_accuracy, loop_until_surpass,local_epochs):
        ...
        console=self.console
            ...
            while True:
                ...
                try:
                    ...
                    # Collect privacy metrics from nodes
                    node_privacy_metrics = []
                    for node in nodes:
                        if node.privacy_metrics:
                            node_privacy_metrics.append(node.privacy_metrics)

                    if not node_privacy_metrics:
                        self.console.print("[red]No privacy metrics received. Skipping round.[/]")
                        break

                    # Compute global privacy averages
                    avg_plm = np.mean([m["PLM"] for m in node_privacy_metrics])
                    avg_gss = np.mean([m["GSS"] for m in node_privacy_metrics])
                    avg_msd = np.mean([m["MSD"] for m in node_privacy_metrics])

                    self.console.print(f"\n[bold red]Global Privacy Metrics for Round {round_num}:[/]")
                    self.console.print(f"🔒 Average Privacy Leakage Metric (PLM): {avg_plm:.4f}")
                    self.console.print(f"🔒 Average Gradient Similarity Score (GSS): {avg_gss:.4f}")
                    self.console.print(f"🔒 Average Mean Squared Difference (MSD): {avg_msd:.6f}")
        ...
        console.print(f"\n[bold red]Final Privacy Metrics After Training:[/]")
        console.print(f"🔒 Final Privacy Leakage Metric (PLM): {avg_plm:.4f}")
        console.print(f"🔒 Final Gradient Similarity Score (GSS): {avg_gss:.4f}")
        console.print(f"🔒 Final Mean Squared Difference (MSD): {avg_msd:.6f}")
